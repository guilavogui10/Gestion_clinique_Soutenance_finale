...
class ExamenControleur:
    """
    Controleur MVC pour la gestion des examens.
    ...
    def creer_examen(self, examen: Examen) -> tuple:
        """
        Valide et cree un nouvel examen.
        Le DAO met automatiquement a jour le statut de la visite.
        """
    ...
    def modifier_examen(self, examen: Examen) -> tuple:
        """
        Valide et met a jour un examen existant.
        Note: La modification d'un examen ne change PAS le statut de la visite
        car le patient est deja dans son parcours.
        """
        valide, msg = self.valider_examen(examen)
        if not valide:
            return False, msg
    ...
    def obtenir_par_visite(self, code_visite: str):
        """Retourne l examen lie a une visite."""
    ...
    def rechercher_examen(self, critere: str, code_session: str) -> list:
        """Recherche des examens par critere dans une session."""
    ...
    def obtenir_services_lies(self, code_examen: str) -> dict:
        """Retourne les services lies a l examen."""
    ...
    def obtenir_examens_aujourd_hui(self, code_session: str) -> int:
        """Card Examens du Jour : nombre d examens crees aujourd hui."""
    ...
    def obtenir_examens_en_attente(self, code_session: str) -> int:
        """Card Examens en Attente : visites avec statut_patient = Attente examen sans examen enregistre."""
    ...
    def obtenir_examens_par_personnel(self, code_session: str) -> list:
        """Retourne le nombre d examens groupe par personnel."""
    ...
    # =========================================================================
    # MÉTHODES ANALYSES ET TABLEAUX (analogue ConsultationControleur)
    # =========================================================================
    ...
    def obtenir_moyenne_nombre_journalier_par_mois(self, code_session: str) -> dict:
        """
        Alias de compatibilite avec les vues analyse basees sur consultation.
        """
        return self.dao.moyenne_examens_journaliers_mois(code_session)
    ...
    def obtenir_rechercher_entre_dates(self, code_session: str, date_debut, date_fin) -> list:
        return self.rechercher_entre_dates(code_session, date_debut, date_fin)

    def rechercher_par_libelle(self, code_session: str, libelle: str = None) -> list:
        try:
            return self.dao.rechercher_par_libelle(code_session, libelle) or []
        except Exception as e:
            self.logger.error(f"Erreur rechercher_par_libelle: {e}")
            return []

    def obtenir_rechercher_par_libelle(self, code_session: str, libelle: str = None) -> list:
        return self.rechercher_par_libelle(code_session, libelle)
    ...
    def obtenir_nombre_par_mois_filtre(self, code_session: str, libelle: str = None) -> dict:
        try:
            return self.dao.nombre_par_mois_filtre(code_session, libelle) or {}
        except Exception as e:
    ...
    def obtenir_examens_par_patient_par_mois(self, code_session: str, code_patient: str = None) -> dict:
        try:
            return self.dao.examens_par_patient_par_mois(code_session, code_patient) or {}
        except Exception as e:
            self.logger.error(f"Erreur obtenir_examens_par_patient_par_mois: {e}")
            return {}
    ...
    def obtenir_patients_en_attente(self, code_session: str) -> list:
        try:
            return self.dao.patients_en_attente(code_session) or []
        except Exception as e:
            self.logger.error(f"Erreur obtenir_patients_en_attente: {e}")
            return []

    def obtenir_patients_examen(self, code_session: str) -> list:
        try:
            return self.dao.patients_pour_examen(code_session) or []
        except Exception as e:
            self.logger.error(f"Erreur obtenir_patients_examen: {e}")
            return []

    def obtenir_patients_chirurgie(self, code_session: str) -> list:
        try:
            return self.dao.patients_pour_chirurgie(code_session) or []
        except Exception as e:
            self.logger.error(f"Erreur obtenir_patients_chirurgie: {e}")
            return []

    def obtenir_patients_lunette(self, code_session: str) -> list:
        try:
            return self.dao.patients_pour_lunette(code_session) or []
        except Exception as e:
            self.logger.error(f"Erreur obtenir_patients_lunette: {e}")
            return []

    def obtenir_patients_prescription(self, code_session: str) -> list:
        try:
            return self.dao.patients_pour_prescription(code_session) or []
        except Exception as e:
            self.logger.error(f"Erreur obtenir_patients_prescription: {e}")
            return []
