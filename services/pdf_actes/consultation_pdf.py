"""
PDF pour les consultations (fiche info, avec résultat, liste multiple).
"""
from ._base import (
...
class ConsultationPDF:
    ...
    def generer_pdf_consultation(consultation, info_cabinet, chemin_pdf=None):
        ...
        elements.append(Paragraph("RAPPORT DE CONSULTATION", titre_style))
        ...
        section_style = ParagraphStyle(
            ...
            textColor=colors.Color(0.15, 0.38, 0.93), fontName='Helvetica-Bold'
        )

        code             = obtenir_valeur(consultation, 'code', 'N/A')
        date_consultation= obtenir_valeur(consultation, 'date_consultation', 'N/A')
        diagnostique     = obtenir_valeur(consultation, 'diagnostique', 'N/A')
        frais            = obtenir_valeur(consultation, 'frais_consultation', 0)
        statut_facture   = obtenir_valeur(consultation, 'statut_facture', 'N/A')
        patient_nom      = obtenir_valeur(consultation, 'patient_nom', 'N/A')
        patient_prenom   = obtenir_valeur(consultation, 'patient_prenom', 'N/A')
        patient_telephone= obtenir_valeur(consultation, 'patient_telephone', 'N/A')
        patient_adresse  = obtenir_valeur(consultation, 'patient_adresse', 'N/A')
        personnel_nom    = obtenir_valeur(consultation, 'personnel_nom', 'N/A')
        personnel_prenom = obtenir_valeur(consultation, 'personnel_prenom', 'N/A')
        personnel_fonction=obtenir_valeur(consultation, 'personnel_fonction', 'N/A')
    ...
    def generer_pdf_consultation_avec_resultat(consultation, resultat, info_cabinet, chemin_pdf=None,
        ...
        frame_patient = _make_frame("INFORMATIONS PATIENT", [
            [Paragraph("<b>Nom</b>", label_style),      Paragraph(str(_v(consultation,'patient_nom')), info_style)],
            [Paragraph("<b>Prénom</b>", label_style),   Paragraph(str(_v(consultation,'patient_prenom')), info_style)],
            [Paragraph("<b>Téléphone</b>", label_style),Paragraph(str(_v(consultation,'patient_telephone')), info_style)],
            [Paragraph("<b>Adresse</b>", label_style),  Paragraph(str(_v(consultation,'patient_adresse')), info_style)],
        ], [3*cm, 5.5*cm])

        frame_personnel = _make_frame("PERSONNEL SOIGNANT", [
            [Paragraph("<b>Nom</b>", label_style),     Paragraph(str(_v(consultation,'personnel_nom')), info_style)],
            [Paragraph("<b>Prénom</b>", label_style),  Paragraph(str(_v(consultation,'personnel_prenom')), info_style)],
            [Paragraph("<b>Fonction</b>", label_style),Paragraph(str(_v(consultation,'personnel_fonction')), info_style)],
            ["", ""],
        ], [3*cm, 5.5*cm])
        ...
        data_consult = [
            [Paragraph("<b>Code Consultation</b>", label_style), Paragraph(str(_v(consultation,'code')), info_style),
             Paragraph("<b>Date</b>", label_style), Paragraph(str(_v(consultation,'date_consultation')), info_style)],
            [Paragraph("<b>Frais</b>", label_style),
             Paragraph(f"{frais:,.0f} GNF".replace(',', ' '), info_style),
             Paragraph("<b>Statut Facture</b>", label_style), Paragraph(str(_v(consultation,'statut_facture')), info_style)],
            [Paragraph("<b>Diagnostique</b>", label_style), Paragraph(str(_v(consultation,'diagnostique')), info_style), "", ""],
        ]
    ...
    def generer_pdf_consultations_multiples(consultations, info_cabinet, chemin_pdf=None):
        ...
        elements.append(Paragraph(f"Nombre total de consultations : {len(consultations)}", stats_style))
        elements.append(Spacer(1, 0.5*cm))

        if consultations:
            data = [['Code', 'Date', 'Diagnostique', 'Frais', 'Statut']]
            for c in consultations:
                code = obtenir_valeur(c, 'code', 'N/A')
                date_val = obtenir_valeur(c, 'date_consultation')
                date_str = date_val.strftime('%d/%m/%Y') if hasattr(date_val, 'strftime') else str(date_val or '')
                diag = obtenir_valeur(c, 'diagnostique', 'N/A')
                if len(str(diag)) > 30:
                    diag = str(diag)[:30] + "..."
                frais = obtenir_valeur(c, 'frais_consultation', 0)
                statut = obtenir_valeur(c, 'statut_facture', 'N/A')
                data.append([code, date_str, diag, f"{frais} GNF", statut])

            table = Table(data, colWidths=[3*cm, 3*cm, 5*cm, 3*cm, 3*cm])
            table.setStyle(TableStyle([
                ('BACKGROUND',(0,0),(-1,0),colors.Color(0.15,0.38,0.93)),
