...
# ── Géométrie page A5 ─────────────────────────────────────────────────────────
_W, _H  = A5           # 419.53 × 595.28 pt
_FRAME  = 8            # cadre à 8 pt du bord
_MX     = 0.45 * cm    # marge horizontale intérieure au cadre
_MY_BOT = 1.0  * cm    # réserve bas (footer)
_LGAP   = 0.25 * cm    # gap entre les 2 colonnes
_COL_W  = (_W - 2 * (_FRAME + _MX) - _LGAP) / 2   # ≈ 185 pt
_LBL_W  = 2.60 * cm    # largeur colonne labels
_LHGT   = 13           # hauteur d'une ligne (pt)
_CPAD   = 0.32 * cm    # padding interne carte
_FSM    = 7.5          # font small (valeurs)
_FTT    = 8.5          # font titre section
...
class ResultatPDFService:
    ...
    def generer(data: dict, info_cabinet: dict, chemin_pdf: str) -> str:
        ...
        # ── Fond de page très léger (couleur accent ultra-diluée) ─────────────
        ...
        # Zone intérieure blanche (fond propre pour le contenu)
