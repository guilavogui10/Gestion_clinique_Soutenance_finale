...
class RapportFacturePatientPDF:

    @staticmethod
    def generer_pdf_par_date(factures, info_cabinet, chemin_pdf=None):
        """Génère un PDF des factures patient groupées par date."""
        import tempfile
        from collections import defaultdict

        if chemin_pdf is None:
            fd, chemin_pdf = tempfile.mkstemp(suffix=".pdf", prefix="rapport_factures_patient_")
            os.close(fd)
        ...
        elements.append(Paragraph("RAPPORT FINANCIER — FACTURES PATIENTS", titre_style))
        ...
        if not factures:
            ...
            for date_key in sorted(groupes.keys()):
                ...
                date_label = _fmt_date(_v(groupe[0], 'date_facture'))
                ...
                for f in groupe:
                    code = str(_v(f, 'code_facture', 'N/A'))
                    nom = _v(f, 'nom_patient', '')
                    prenom = _v(f, 'prenom_patient', '')
                    patient = f"{nom} {prenom}".strip() or 'N/A'
                    ...
                    statut = str(_v(f, 'statut_facture', '-') or '-')
    ...
    def generer_pdf_date_precise(factures, date_cible, info_cabinet, chemin_pdf=None):
        """Génère un PDF des factures patient pour une date précise."""
        import tempfile

        if chemin_pdf is None:
            fd, chemin_pdf = tempfile.mkstemp(suffix=".pdf", prefix="rapport_fp_date_")
            os.close(fd)
        ...
        def _get_date_key(f):
            d = f.get('date_facture') if isinstance(f, dict) else getattr(f, 'date_facture', None)
        ...
        titre_style = ParagraphStyle(
            'RptFpDTitle', parent=styles['Heading1'],
            fontSize=16, spaceAfter=12, alignment=1, textColor=bleu
        )
        sous_titre_style = ParagraphStyle(
            'RptFpDSub', parent=styles['Normal'],
            fontSize=12, spaceAfter=16, alignment=1, textColor=bleu
        )
        date_gen_style = ParagraphStyle(
            'RptFpDDateGen', parent=styles['Normal'],
            fontSize=10, alignment=2, textColor=colors.gray
        )
        cell_style = ParagraphStyle(
            'RptFpDCell', parent=styles['Normal'],
            fontSize=8, leading=10
        )
        header_cell_style = ParagraphStyle(
            'RptFpDHdr', parent=styles['Normal'],
            fontSize=9, textColor=colors.white, fontName='Helvetica-Bold'
        )
        total_label_style = ParagraphStyle(
            'RptFpDTotLbl', parent=styles['Normal'],
            fontSize=9, fontName='Helvetica-Bold', textColor=bleu
        )
        total_val_style = ParagraphStyle(
            'RptFpDTotVal', parent=styles['Normal'],
            fontSize=9, fontName='Helvetica-Bold', textColor=bleu, alignment=2
        )
        ...
        if not filtrees:
            elements.append(Paragraph(
                f"Aucune facture pour le {date_affichee}.", styles['Normal']
            ))
        else:
            header_row = [
                Paragraph("<b>Code Facture</b>", header_cell_style),
