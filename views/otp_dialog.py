...
otp_dialog.py
-------------
Dialogue moderne pour la saisie du code OTP lors de l'authentification à deux facteurs.
Design élégant avec icônes Font Awesome, timer de compte à rebours et possibilité de renvoyer le code.
"""
...
class OTPDialog(QDialog):
    """
    Dialogue moderne pour la saisie du code OTP.
    Affiche un timer de 5 minutes et permet de renvoyer le code.
    """
    
    resend_requested = Signal()
    
    def __init__(self, masked_email: str, parent=None):
        super().__init__(parent)
        self.masked_email = masked_email
        self.remaining_seconds = 300  # 5 minutes
        self.otp_code = ""
        
        self.setWindowTitle("Vérification à deux facteurs")
        self.setModal(True)
    ...
    def _build_header(self, parent_layout):
        ...
        # Icône shield avec cercle de fond
        ...
        icon_label = QLabel()
        icon_label.setPixmap(qta.icon("fa5s.shield-alt", color="white").pixmap(40, 40))
        ...
        # Titre
        title = QLabel("Vérification à deux facteurs")
    ...
    def _build_message(self, parent_layout):
        ...
        # Icône email
        email_icon = QLabel()
        email_icon.setPixmap(qta.icon("fa5s.envelope", color="#0EA5E9").pixmap(20, 20))
        email_icon.setStyleSheet("background: transparent; border: none;")
        msg_layout.addWidget(email_icon, alignment=Qt.AlignTop)
        
        # Texte
        message = QLabel(
            f"Un code de vérification à 6 chiffres a été envoyé à :\n"
            f"<b>{self.masked_email}</b>\n\n"
            f"Veuillez saisir ce code pour terminer la connexion."
        )
    ...
    def _build_otp_inputs(self, parent_layout):
        # Label
        label = QLabel("Code de vérification")
        label.setStyleSheet("""
            font-size: 13px;
            font-weight: 600;
            color: #475569;
    ...
    def _build_buttons(self, parent_layout):
        # Bouton Vérifier
        self.btn_verify = QPushButton("  Vérifier le code")
