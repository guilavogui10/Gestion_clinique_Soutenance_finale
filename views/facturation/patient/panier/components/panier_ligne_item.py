"""
Composant ligne individuelle du panier.
Responsabilité : Affichage d'une ligne produit avec bouton suppression.
"""
...
class PanierLigneItem:
    """Crée une ligne visuelle pour un produit dans le panier."""
    
    def __init__(self, vert_principal: str, rouge: str):
        self.vert_principal = vert_principal
        self.rouge = rouge
    
    def create(self, designation: str, quantite: int, prix: float, 
               date_exp: str, code_panier: str, on_delete_callback) -> QFrame:
        """
        Crée une ligne panier avec toutes les informations - Style E-COMMERCE MODERNE.
        """
        ...
        # Stocker les données dans le widget pour calcul du total
        ligne.code_panier = code_panier
        ...
        # Icône produit avec background coloré
        ...
        icon_lbl = QLabel()
        icon_lbl.setPixmap(
            qta.icon("fa5s.capsules", color=c['text_inverse']).pixmap(QSize(24, 24))
        )
        ...
        detail_lbl = QLabel(
            ...
            f"<span style='color: {c['warning']}; font-weight: bold;'>{date_exp}</span>"
            .replace(",", " ")
        )
        ...
        # Sous-total avec style mis en valeur
        total_container = QVBoxLayout()
        total_container.setSpacing(2)
        
        total_label = QLabel("Sous-total")
        ...
        total_value = QLabel(f"{sous_total:,.0f} GNF".replace(",", " "))
        ...
        total_container.addWidget(total_value)
        ...
        btn_del.setStyleSheet(f"""
            ...
            QPushButton:hover {{
                background: {c['danger']}cc;
            }}
            QPushButton:pressed {{
                background: {c['danger']}99;
            }}
        """)
        btn_del.setCursor(Qt.PointingHandCursor)
        btn_del.setToolTip("Supprimer cet article du panier")
