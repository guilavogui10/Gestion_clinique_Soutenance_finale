# """
# Tableau commandes lunettes :
# - filtre par recherche, date (simple et intervalle)
# - bouton imprimer
# - tableau des commandes avec alternance de lignes
# - graphe bar  : commandes par mois (global)
# - graphe bar  : commandes du patient sélectionné (accent)
# """
...
# class TableauLunetteView(QWidget):
#     def __init__(self, lunette_ctrl, code_session):
#         super().__init__()
#         self.ctrl              = lunette_ctrl
#         self.code_session      = code_session
#         self.commandes         = []
#         self.filtered_commandes = []
#         self._init_ui()
#         self.charger_commandes(code_session)
...
#         self._tbl_header      = self._creer_header("Tableau commandes lunettes", "fa5s.glasses")
...
#             ("fa5s.barcode",      "Code"),
#             ("fa5s.user",         "Patient"),
#             ("fa5s.glasses",      "N° Cadre"),
#             ("fa5s.eye",          "N° Verre"),
#             ("fa5s.money-bill",   "Prix"),
#             ("fa5s.calendar-alt", "Date commande"),
#         ]
...
#         # Graphe 1 : Commandes par mois
#         self._graph_title1 = QLabel("Commandes lunettes par mois")
...
#         # Graphe 2 : Commandes du patient sélectionné
#         self._graph_title2 = QLabel("Commandes du patient")
...
#         _header_icons = [
#             "fa5s.barcode", "fa5s.user", "fa5s.glasses",
#             "fa5s.eye", "fa5s.money-bill", "fa5s.calendar-alt"
#         ]
...
#         self.code_session = code_session
#         try:
#             self.commandes = self.ctrl.lister_commandes(code_session) or []
#         except Exception:
#             self.commandes = []
...
#                     a_commande = patient.get('a_consulte', 0)
...
#             print(f"[TableauLunetteView] Erreur _remplir_combo_patients: {e}")
...
#         else:
#             self.search_input.setPlaceholderText("Rechercher par code, patient, cadre ou verre...")
#         self._actualiser_filtrage()
...
#         if filtre == "Tous":
#             self.filtered_commandes = (
#                 self._filtrer_par_texte(self.commandes, texte) if texte
#                 else self.commandes
#             )
...
#                     )
#                     self.filtered_commandes = result or []
...
#         self._remplir_table(self.filtered_commandes)
#         self._actualiser_graphique()

#     def _filtrer_par_texte(self, commandes, texte: str) -> list:
...
#                 str(getattr(c, 'numero_cadre', '')),
#                 str(getattr(c, 'numero_verre', '')),
#                 str(getattr(c, 'prix', '')),
#                 self._format_date(getattr(c, 'date_commande', '')),
#             ]
...
#     def _remplir_table(self, commandes):
#         """Remplit le tableau — accès par getattr uniquement, jamais par dict."""
...
#             self.table.setItem(row, 2, QTableWidgetItem(str(getattr(c, 'numero_cadre', '—'))))
#             self.table.setItem(row, 3, QTableWidgetItem(str(getattr(c, 'numero_verre', '—'))))
#             self.table.setItem(row, 4, QTableWidgetItem(str(getattr(c, 'prix', '—'))))
#             self.table.setItem(
#                 row, 5,
#                 QTableWidgetItem(self._format_date(getattr(c, 'date_commande', '—')))
#             )

#     # =========================================================================
#     # GRAPHE 1 : COMMANDES PAR MOIS (BAR HORIZONTAL)
#     # =========================================================================
...
#             ax.set_xlabel("Commandes", fontsize=9, fontweight="bold", color=MC.TEXT_PRIMARY)
...
#             self.graph_canvas.draw()
#         except Exception as e:
#             print(f"[TableauLunetteView] Erreur _actualiser_graphique: {e}")

#     # =========================================================================
#     # GRAPHE 2 : COMMANDES DU PATIENT (BAR HORIZONTAL — ACCENT)
#     # =========================================================================
...
#             data = self.ctrl.obtenir_commandes_par_patient_par_mois(
...
#             self.graph_patient_canvas.draw()
#         except Exception as e:
#             print(f"[TableauLunetteView] Erreur _actualiser_graphique_patient: {e}")
...
#     def _get_nom_patient(self, code_commande: str) -> str:
#         try:
#             data = self.ctrl.obtenir_commande_complete(code_commande)
#             if not data:
#                 return "—"
...
#             from services.lunette_pdf_service import LunettePDFService
#             import os

#             commandes_a_imprimer = (
#                 self.filtered_commandes if self.filtered_commandes else self.commandes
#             )

#             if not commandes_a_imprimer:
#                 QMessageBox.warning(
#                     self, "Aucune donnée",
#                     "Aucune commande à imprimer avec les filtres actuels."
#                 )
#                 return
...
#             nom_fichier_def = f"commandes_lunettes_{timestamp}.pdf"
...
#             success = LunettePDFService.generer_pdf_commandes_filtrees(
#                 commandes_a_imprimer, filtres_appliques, chemin_pdf, info_cabinet
#             )
...
#         except Exception as e:
#             from PySide6.QtWidgets import QMessageBox
#             QMessageBox.critical(self, "Erreur inattendue", f"{str(e)}")