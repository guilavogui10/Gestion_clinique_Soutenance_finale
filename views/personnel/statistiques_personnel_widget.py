...
class StatistiquesPersonnelWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        scroll.setStyleSheet(f"""
            ...
            QScrollArea > QWidget {{ background: {c['bg_main']}; }}
        """)
    ...
    def _create_kpi_card(self, nom, nombre, pct, couleur, icon_name):
        ...
        top.setSpacing(6)
        icon_label = QLabel()
        icon_label.setPixmap(qta.icon(icon_name, color=couleur).pixmap(16, 16))
        top.addWidget(icon_label)

        nom_label = QLabel(nom)
        nom_label.setStyleSheet(f"font-size: 9px; color: {c['text_secondary']}; font-weight: 500;")
        ...
        nombre_label.setStyleSheet(f"font-size: 24px; font-weight: bold; color: {c['text_primary']};")
        layout.addWidget(nombre_label)

        # Pourcentage
        pct_label = QLabel(pct)
        pct_label.setStyleSheet(f"font-size: 8px; color: {c['text_muted']};")
    ...
    def _create_total_card(self):
        ...
        titre.setStyleSheet(f"font-size: 8px; color: {c['text_secondary']}; font-weight: 500;")
        layout.addWidget(titre)

        nombre = QLabel("31")
        nombre.setStyleSheet(f"font-size: 26px; font-weight: bold; color: {c['primary']};")
        layout.addWidget(nombre)

        pct = QLabel("100%")
        pct.setStyleSheet(f"font-size: 10px; color: {c['primary']}; font-weight: 600;")
        layout.addWidget(pct)

        effectifs = QLabel("Effectifs totaux")
        effectifs.setStyleSheet(f"font-size: 8px; color: {c['text_muted']};")
        layout.addWidget(effectifs)

        icon_label = QLabel()
        icon_label.setPixmap(qta.icon('fa5s.users', color=c['primary']).pixmap(14, 14))
    ...
    def _create_responsables_section(self):
        ...
        titre.setStyleSheet(
            f"font-size: 14px; font-weight: bold; color: {c['text_primary']}; background: transparent;"
        )
        layout.addWidget(titre)
    ...
    def _create_resp_card(self, fonction, nb_emp, couleur, icon_name, nom, titre, email, tel, date, lieu, photo_path):
        ...
        header = QHBoxLayout()
        icon_label = QLabel()
        icon_label.setPixmap(qta.icon(icon_name, color=couleur).pixmap(22, 22))
        header.addWidget(icon_label)

        fonction_label = QLabel(fonction)
        fonction_label.setStyleSheet(f"font-size: 13px; font-weight: bold; color: {c['text_primary']};")
        ...
        line.setStyleSheet(f"background: {c['border_light']};")
        ...
        if photo_path:
            ...
            if os.path.exists(full_photo_path):
                ...
                if not pixmap.isNull():
                    ...
                    photo.setStyleSheet(f"border-radius: 35px; border: 2px solid {c['bg_card']};")
                    photo_loaded = True
        ...
        nom_label = QLabel(nom)
        nom_label.setStyleSheet(f"font-size: 12px; font-weight: bold; color: {c['text_primary']};")
        info_layout.addWidget(nom_label)

        titre_label = QLabel(titre)
        titre_label.setStyleSheet(f"font-size: 10px; color: {c['text_secondary']};")
    ...
    def _add_info_line(self, parent_layout, icon_name, text):
        ...
        icon_label = QLabel()
        icon_label.setPixmap(qta.icon(icon_name, color=c['text_muted']).pixmap(10, 10))
        row.addWidget(icon_label)

        text_label = QLabel(text)
        text_label.setStyleSheet(f"font-size: 9px; color: {c['text_secondary']};")
    ...
    def _create_info_generales(self):
        ...
        header = QHBoxLayout()
        icon_label = QLabel()
        icon_label.setPixmap(qta.icon('fa5s.info-circle', color=c['primary']).pixmap(22, 22))
        header.addWidget(icon_label)

        titre = QLabel("Informations générales")
        titre.setStyleSheet(f"font-size: 13px; font-weight: bold; color: {c['text_primary']};")
    ...
    def _add_stat_row(self, parent_layout, icon_name, label, value):
        ...
        icon_label = QLabel()
        icon_label.setPixmap(qta.icon(icon_name, color=c['text_secondary']).pixmap(12, 12))
        row.addWidget(icon_label)

        label_widget = QLabel(label)
        label_widget.setStyleSheet(f"font-size: 10px; color: {c['text_secondary']};")
        row.addWidget(label_widget)
        row.addStretch()

        value_widget = QLabel(value)
        value_widget.setStyleSheet(f"font-size: 11px; font-weight: bold; color: {c['text_primary']};")
    ...
    def charger_donnees(self):
        ...
        # Couleurs sémantiques des catégories médicales (intentionnellement fixes)
        ...
        if autres > 0:
            ...
            c = theme_manager.colors()
            card_autres = self._create_kpi_card("Autres", str(autres), pct_text, c['text_muted'], "fa5s.users")
        ...
        if autres > 0:
            ...
            card_autres = self._create_resp_card(
                "Autres", nb_emp, c['text_muted'], "fa5s.users",
    ...
    def apply_theme(self):
        """Recharge les données pour appliquer le nouveau thème."""
        c = theme_manager.colors()
        self.setStyleSheet(f"StatistiquesPersonnelWidget {{ background: {c['bg_main']}; }}")
        if hasattr(self, '_scroll'):
            self._scroll.setStyleSheet(
                f"QScrollArea {{ background: {c['bg_main']}; border: none; }}"
                f"QScrollArea > QWidget {{ background: {c['bg_main']}; }}"
            )
        try:
