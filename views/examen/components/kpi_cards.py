...
class KpiCardsSection(QWidget):
    ...
    def init_ui(self):
        ...
        self.card_today = KpiCard("Examens du jour", "fa5s.calendar-day", "info")
        self.card_session = KpiCard("Session en cours", "fa5s.flask", "success")
    ...
    def rafraichir(self, code_session):
        ...
        today_count = self.ctrl.obtenir_examens_aujourd_hui(code_session)
        self.card_today.set_value(today_count, "+12% vs hier")

        session_count = self.ctrl.obtenir_total_examens_session(code_session)
        self.card_session.set_value(session_count, "Examens totaux")

        waiting_count = self.ctrl.obtenir_examens_en_attente(code_session)
        self.card_waiting.set_value(waiting_count, "Sans examen")

        amount_today = self.ctrl.obtenir_montant_aujourd_hui(code_session)
        self.card_amount_today.set_value(f"{amount_today:,.0f} GNF".replace(",", " "), "Frais examens")
