...
class PatientCard(QFrame):
    ...
    def _setup_ui(self):
        ...
        self._badge = QLabel("Attente Examen")
...
class PatientsAttenteExamenView(QWidget):
    ...
    def _on_proceder(self, patient):
        """Ouvre ExamenFormDialog pré-rempli avec les données du patient."""
        ...
        if dialog.exec():
            ...
            self.charger_patients()
            # Émettre le signal pour rafraîchir la vue parent (ExamenView)
            self.examen_cree.emit()
...
class PatientsAttenteDialog(QDialog):
    ...
    Appelé depuis ExamenView._ouvrir_formulaire.

    Usage
    -----
    dialog = PatientsAttenteDialog(self.ctrl, self.code_session, parent=self)
    dialog.exec()
    self.charger_examens(self.code_session)   # rafraîchit la liste principale
    """
