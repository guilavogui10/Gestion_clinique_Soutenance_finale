...
class KpiCardsSection(QWidget):
    ...
    def init_ui(self):
        ...
        self.card_today = KpiCard("Consultations du jour", "fa5s.calendar-day", "info")
        self.card_session = KpiCard("Session en cours", "fa5s.users", "success")
    ...
    def rafraichir(self, code_session):
        ...
        self.card_amount_today.set_value(f"{amount_today:,.0f} GNF".replace(",", " "), "Frais consultations")

        amount_session = self.ctrl.obtenir_montant_session(code_session)
        self.card_amount_session.set_value(f"{amount_session:,.0f} GNF".replace(",", " "), "Total annee")
