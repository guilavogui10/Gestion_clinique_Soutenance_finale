"""
Onglet Historique Patient — intégré dans la vue Consultation.

Architecture :
  • Gauche  : timeline scrollable des consultations du patient sélectionné
  • Droite  : détail de la consultation sélectionnée, composé de cartes blanches
"""
...
class HistoriqueConsultationView(QWidget):
    """Vue onglet — historique des consultations d'un patient."""
    ...
    def _charger_patients(self):
        ...
        for p in patients:
            ...
            has_csl = p.get("a_consulte", 0) if isinstance(p, dict) else 0
    ...
    def _charger_historique(self, code_patient: str):
        ...
        if not rows:
            self._show_empty_state("fa5s.folder-open", "Aucune consultation trouvée pour ce patient")
            return
    ...
    def _build_timeline_card(self, row, is_first=False):
        ...
        date     = self._g(row, "date_consultation")
        diag     = self._g(row, "diagnostique", "-")
        ...
        diag_row.addWidget(ico_diag)
        diag_row.addWidget(diag_lbl, 1)
        cl.addLayout(diag_row)
    ...
    def _show_detail(self, row):
        """Charge et affiche le détail complet d'une consultation."""
        self._clear_detail()
        c = theme_manager.colors()

        code = self._g(row, "code", "")
        try:
            detail = self.ctrl.obtenir_consultation_complete(code) or {}
        except Exception:
            detail = {}
        ...
        # Champs consultation
        date_csl    = g("date_consultation")
        diagnostique= g("diagnostique", "-")
        frais       = g("frais_consultation", 0)
        statut_fact = g("statut_facture", "-")
        chiurgie    = g("chiurgie", "Non")
        prescription= g("prescription_produit", "Non")
        # Champs patient
        p_nom    = g("patient_nom", "")
        ...
        date_str  = date_csl.strftime("%d/%m/%Y %H:%M") if hasattr(date_csl, "strftime") else str(date_csl or "-")
        ...
        row_top_l.addWidget(
            self._card_personnel_header(per_nom, per_prenom, per_fonc, dr_name,
                                        diagnostique, date_str, c)
        )
        self._detail_layout.addWidget(row_top)

        # ── Ligne 2 : grille d'informations ──────────────────────────────
        infos = [
            ("fa5s.barcode",             "Code consultation",  code),
            ("fa5s.calendar-alt",        "Date",               date_str),
            ("fa5s.stethoscope",         "Diagnostic",         diagnostique),
            ("fa5s.money-bill-wave",     "Frais",              frais_str),
            ("fa5s.file-invoice-dollar", "Statut facture",     statut_fact),
            ("fa5s.cut",                 "Chirurgie",          chiurgie or "Non"),
            ("fa5s.pills",               "Prescription",       prescription or "Non"),
        ]
    ...
    def _card_personnel_header(self, nom, prenom, fonction, dr_name, diagnostique, date_str, c):
        card = self._white_card()
        cl = QHBoxLayout(card)
        cl.setContentsMargins(18, 16, 18, 16)
        cl.setSpacing(16)
        ...
        if diagnostique and diagnostique != "-":
            txt = str(diagnostique)[:40]
            sub_row.addWidget(self._icon_text("fa5s.stethoscope", txt, c))
        if date_str:
            sub_row.addWidget(self._icon_text("fa5s.calendar-check", date_str, c))
        sub_row.addStretch()
        info_l.addLayout(sub_row)
