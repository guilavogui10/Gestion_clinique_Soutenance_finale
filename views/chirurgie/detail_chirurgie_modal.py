...
class DetailsChirurgieModal(QDialog):
    """
    Modal d affichage complet d une chirurgie.
    Layout :
        - Header  : infos cabinet + logo
        - Bandeau : patient + code + date + medecin
        - Corps   : 2 colonnes
            Gauche  : Libelle (zone large)
            Droite  : Infos patient + Facturation + Codes
        - Footer  : Fermer + Imprimer
    """

    def __init__(self, parent, code_chururgie: str, ctrl):
        super().__init__(parent)
        self.ctrl        = ctrl
        self.code_chururgie = code_chururgie

        self.data        = self.ctrl.obtenir_chururgie_complete(code_chururgie) or {}
        self.info_cabinet = (
            self.ctrl.get_cabinet_info()
            if hasattr(self.ctrl, "get_cabinet_info") else {}
        )

        self.setWindowTitle(f"Dossier Chirurgie - {code_chururgie}")
    ...
    def _setup_header(self, layout):
        ...
        # Badge "CHIRURGIE"
        badge = QLabel("  CHIRURGIE  ")
        badge.setStyleSheet(f"""
            background-color: {MC.DANGER_BG};
            color: {MC.DANGER};
    ...
    def _setup_bandeau(self, layout):
        ...
        lbl_code = QLabel(f"N° {self.code_chururgie}")
        lbl_code.setStyleSheet(
            f"color: {MC.DANGER}; font-weight: bold; font-size: 13px;"
    ...
    def _bloc_facturation(self, frais, statut) -> QFrame:
        ...
        lbl_fk = QLabel("Frais de chirurgie :")
    ...
    def _setup_footer(self, layout):
        ...
        btn_close.setStyleSheet(f"""
            ...
            QPushButton:hover {{ background-color: {MC.HOVER}; }}
        """)
        ...
        btn_print.setStyleSheet(f"""
            QPushButton {{
                background-color: {MC.DANGER}; color: {MC.TEXT_INVERSE};
                border-radius: 8px; font-weight: bold;
                padding: 8px 25px; border: none;
            }}
            QPushButton:hover {{ background-color: {MC.DANGER}; opacity: 0.85; }}
        """)
